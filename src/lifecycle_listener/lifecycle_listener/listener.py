...
class LifecycleListener(Node):
  ...
  def listener_cb(self,msg):
    ...
    if self.sub_ is None:
      self.get_logger().info('Lifecycle listener is currently inactive. Messages are not listened.')
    else:
      self.get_logger().debug(f'Lifecycle listener is active. Listening: [{msg.data}]')
  ...
  def on_cleanup(self, state: State) -> TransitionCallbackReturn:
    ...
    in "inactive" depending on the return value.
      ...
      TransitionCallbackReturn.ERROR or any uncaught exceptions to "errorprocessing"
    """
    self.destroy_publisher(self.sub_)
